...
public class NormalBow extends BowItem {

	public NormalBow(Settings settings) {
		super(settings);
	  ...
	  public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
	    ...
	    if (!world.isClient) {
	      ...
	      projectileEntity.setProperties((Entity)playerEntity, playerEntity.pitch, playerEntity.yaw, 0.0F, f * 3.0F, 1.0F);
	        projectileEntity.setDamage(projectileEntity.getDamage() + projectileEntity.getDamage() * 0.75D + 0.75D); 
	      if (f == 1.0F)
