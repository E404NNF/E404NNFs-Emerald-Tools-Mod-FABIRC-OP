...
public class QuartzBow extends BowItem {

	public QuartzBow(Settings settings) {
		super(settings);
	...
	public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
	    ...
	    if (!world.isClient) {
	      ...
	      projectileEntity.setProperties((Entity)playerEntity, playerEntity.pitch, playerEntity.yaw, 0.0F, f * 3.0F, 1.0F);
	        projectileEntity.setDamage(projectileEntity.getDamage() + projectileEntity.getDamage() * 1.6875D + 1.6875D); 
	      if (f == 1.0F)
